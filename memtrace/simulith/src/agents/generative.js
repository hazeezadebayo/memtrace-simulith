...
export async function proposeGenerativePersonas(scenario, evidence, existingPersonas = [], emit = () => { }, customPersonas = []) {
  ...
  for (let i = 0; i < neededCount; i++) {
    ...
    const prompt = `
      ...
      4. Do NOT include any extra properties not defined in the JSON schema below.
    ...
    if (parsed) {
      ...
      emit('llm_error', `Persona JSON parse failed or LLM returned empty. Injecting safe fallback persona.`);
      personas.push({
        name: `Fallback Persona ${personas.length + 1}`,
        age: 40,
        gender: "Neutral",
        race: "Diverse",
        location: "Global",
        ...
        bio: "An analytical fallback persona injected due to a transmission error in generation, ensuring the simulation proceeds without missing a stakeholder.",
        riskToleranceProfile: "moderate",
        evidenceRequirementProfile: "high",
        noveltySeekingProfile: "moderate",
        clarityNeedProfile: "high"
      });
    }
