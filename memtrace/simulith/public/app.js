  ...
  results.querySelectorAll('[data-resimulate-branch]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!response.ok) {
          let errText = "Failed to resimulate branch.";
          try {
        ...
        const updated = data.updatedBranch;
        ...
        if (existingCard) {
          ...
          tmp.innerHTML = renderBranch(updated);
          const newCard = tmp.firstElementChild;
          existingCard.replaceWith(newCard);

          // Re-attach record-outcome listener on the new card
          newCard.querySelectorAll('[data-record-outcome]').forEach(ob =>
            ob.addEventListener('click', () => result && ob.dispatchEvent(new Event('click')))
          );

          // Re-attach resimulate listener on the new card
          newCard.querySelectorAll('[data-resimulate-branch]').forEach(rb => {
            rb.addEventListener('click', async () => {
              if (currentMode !== 'council') {
                console.warn("Re-simulate is only allowed in Council mode.");
                return;
              }
              const newBranchId = rb.dataset.resimulateBranch;
              const newInput = document.getElementById(`counter-input-${newBranchId}`);
              const newEvidence = newInput ? newInput.value.trim() : '';
              if (!newEvidence) { alert('Please provide a counter-argument or new evidence to re-simulate this branch.'); return; }
              rb.innerText = 'SIMULATING...';
              rb.disabled = true;
              logConsole(`RESIMULATING BRANCH: ${newBranchId} WITH NEW EVIDENCE`, 'system');
              try {
                const r2 = await fetch(`/api/v4/runs/${result.id}/branches/${newBranchId}/resimulate`, {
                  method: 'POST', headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ newEvidence })
                });
                if (!r2.ok) {
                ...
                logConsole(`RESIMULATE ERROR: ${e2.message}`, 'error');
                rb.innerText = 'RE-SIMULATE';
                rb.disabled = false;
              }
            });
          });
        ...
        logConsole(`BRANCH ${branchId} RESIMULATED.`, 'system');
      } catch (err) {
        logConsole(`RESIMULATE ERROR: ${err.message}`, 'error');
